    ...
    def build(self, input_shape: tf.TensorShape):
        ...
        if self.learnable:
            self.freqs = self.add_weight(
                name="freqs", shape=shape, initializer=self.initializer, trainable=True
            )
        else:
            self.freqs = tf.constant(
                2.0
                ** tf.range(1, self.num_frequencies + 1, dtype=tf.float32)[
                    tf.newaxis, :
                ]
            )
...
class DenseNetwork(tf.keras.Model):
    ...
    def build(self, input_shape):
        ...
        # Positional Encoding
        if (
            self.positional_encoding_frequencies
            and self.positional_encoding_frequencies > 0
        ):
    ...
    def train_model(
        ...
        if optimizer not in ["adam", "sgd", "rmsprop"]:
            raise ValueError(
                "Unsupported optimizer. Supported optimizers are: 'adam', 'sgd', 'rmsprop'."
            )
        if optimizer == "sgd":
        ...
        if log:
            ...
            tensorboard_callback = tf.keras.callbacks.TensorBoard(
                log_dir=log_dir, write_steps_per_second=True
            )
    ...
    def plot_training_history(self) -> None:
        ...
        if self.history is None:
            raise ValueError(
                "The model has no training history. Train the model using 'train_model' method first."
            )
        ...
        plt.plot(
            valid_freq * np.arange(tot_valid),
...
class DeepONet(tf.keras.Model):
    ...
    def train_model(
        ...
        self.history = self.fit(
            ...
            batch_size=batch_size,
            verbose=verbose,
            validation_data=([mu_val, X_val], y_val),
            validation_freq=validation_freq,
            callbacks=callbacks,
        )
