...
def _find_id(mu: np.ndarray, data_folder: str = "data") -> int:
    """
...
def main():
    ...
    parser.add_argument(
        "--sample_from_test_set",
        action="store_true",
        help="If set, sample from the test set; otherwise, sample from the entire dataset.",
    )
    ...
    if target == "potential":
        ...
        with h5py.File(fom_file, "a") as file:
            ...
            if "normal_se" in file:
                del file["normal_se"]
            file["normal_se"] = (
                y_pred[0][nan_mask] - file["normal_derivatives_plate"][:]
            ) ** 2
            if "normal_ae" in file:
                del file["normal_ae"]
            file["normal_ae"] = np.abs(
                y_pred[0][nan_mask] - file["normal_derivatives_plate"][:]
            )
