...
- ``--batch_size``: Batch size for training (default: :math:`8`).
- ``--rescale_output``: Factor to scale the output during training (default: :math:`1.0`, no scaling).
"""
...
def train(
    ...
    warmup_callback = tf.keras.callbacks.LearningRateScheduler(
...
def main():
    ...
    parser.add_argument(
        "--r", type=int, default=20, help="Low-rank dimension of the DeepONet."
    )
    parser.add_argument(
        "--seed", type=int, default=40, help="Random seed for data splitting."
    )
    parser.add_argument(
        "--target",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Factor to scale the output during training (default: 1.0, no scaling).",
    )
    ...
    run_on_device(
        ...
        rescale_output=args.rescale_output,
    )
